					...
					do
						...
						do
							...
							do
								for normalized in "${NORMALIZE[@]}"
								do
									eval "$PYTHON $BASEDIR/$SCRIPT --lat_lon=$LAT_LON --name=$NAME --from_date=$FROM_DATE --model=$model --days_threshold=$day_threshold $days_in_out $fill_missing $reducer $class_mode $normalized $rs_train_size $rs_iter"
								done
							done
						done
					done
