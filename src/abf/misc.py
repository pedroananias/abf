...
def concordance_measures(cm, y_true, y_pred):
    ...
    kappa, vkappa = float(kappa if not math.isnan(kappa) else 0.0), float(
        vkappa if not math.isnan(vkappa) else 0.0
    )
    ...
    tau, vtau = float(tau if not math.isnan(tau) else 0.0), float(
        vtau if not math.isnan(vtau) else 0.0
    )
