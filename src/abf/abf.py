...
class Abf:
    ...
    def __init__(
        ...
        self.grid_size = (
            int(grid_size) - 1 if (int(grid_size) % 2) == 0 else int(grid_size)
        )
        ...
        self.shapefile = (
            ee.FeatureCollection(self.shapefile_url) if self.shapefile_url else None
        )
        ...
        if not test_mode:
            ...
            if scaler == "minmax":
                ...
                self.scaler = preprocessing.RobustScaler(
                    quantile_range=(25, 75), copy=True
                )
            else:
            ...
            if from_date is None:
                self.dates_timeseries[1] = dt.fromtimestamp(
            ...
            self.dates_timeseries[0] = self.dates_timeseries[1] - td(
            ...
            collection, collection_water = gee.get_sensor_collections(
                geometry=self.geometry,
                sensor=self.sensor,
                dates=[
                    dt.strftime(self.dates_timeseries[0], "%Y-%m-%d"),
                    dt.strftime(self.dates_timeseries[1], "%Y-%m-%d"),
                ],
            )

            # create usefull time series
            if self.shapefile:
                self.collection = collection.map(
                    lambda image: image.clip(self.shapefile)
                )
            else:
                self.collection = collection
            self.collection_water = collection_water
            self.dates_timeseries_interval = misc.remove_duplicated_dates(
                [
                    dt.strptime(d, "%Y-%m-%d")
                    for d in self.collection.map(
            ...
            self.sample_total_pixel = gee.get_image_counters(
                image=self.sample_clip.select("constant"),
                geometry=self.geometry,
                scale=self.scale,
            )["constant"]
            coordinates_min, coordinates_max = gee.get_image_min_max(
                image=self.sample_clip, geometry=self.geometry, scale=self.scale
            )
            self.sample_lon_lat = [
                [
                    float(coordinates_min["latitude"]),
                    float(coordinates_min["longitude"]),
                ],
                [
                    float(coordinates_max["latitude"]),
                    float(coordinates_max["longitude"]),
                ],
            ]
            ...
            print(
                ...
                + " meters, pixels="
                + str(self.sample_total_pixel)
                + ", initial_date='"
                + self.dates_timeseries[0].strftime("%Y-%m-%d")
                + "', end_date='"
                + self.dates_timeseries[1].strftime("%Y-%m-%d")
                + "', interval_images='"
                + str(self.collection.size().getInfo())
                + "', interval_unique_images='"
                + str(len(self.dates_timeseries_interval))
                + "', water_mask_images='"
                + str(self.collection_water.size().getInfo())
                + "', grid_size='"
                + str(self.grid_size)
                + "', days_in='"
                + str(self.days_in)
                ...
                + str(self.morph_op)
                + "', morph_op_iters='"
                + str(self.morph_op_iters)
                + "', convolve='"
                + str(self.convolve)
                + "', convolve_radius='"
                + str(self.convolve_radius)
                + "', scaler='"
                + str(self.scaler_str)
                + "', model='"
                + str(self.model)
                + "', fill_missing='"
                + str(self.fill_missing)
                + "', reducer='"
                + str(self.reducer)
                + "', normalized='"
                + str(self.normalized)
                ...
                + str(self.propagate)
                + "', rs_train_size='"
                + str(self.rs_train_size)
                + "', rs_iter='"
                + str(self.rs_iter)
                + "', pca_size='"
                + str(self.pca_size)
                + "', attr_lat_lon='"
                + str(self.attribute_lat_lon)
                + "', attr_doy='"
                + str(self.attribute_doy)
                + "'"
            )
    ...
    def create_water_mask(self, morph_op: str = None, morph_op_iters: int = 1):
        ...
        if not morph_op is None and morph_op != "":
            if morph_op == "closing":
                water_mask = water_mask.focal_max(
                    kernel=ee.Kernel.square(radius=1), iterations=morph_op_iters
                ).focal_min(
                    kernel=ee.Kernel.circle(radius=1), iterations=morph_op_iters
                )
            elif morph_op == "opening":
                water_mask = water_mask.focal_min(
                    kernel=ee.Kernel.square(radius=1), iterations=morph_op_iters
                ).focal_max(
                    kernel=ee.Kernel.circle(radius=1), iterations=morph_op_iters
                )
    ...
    # clipping image
    def clip_image(
        self, image: ee.Image, scale: int = None, geometry: ee.Geometry = None
    ):
        ...
        if self.sensor_params["scale"] > self.scale:
            return clip.resample("bicubic").reproject(
                crs=image.projection(), scale=self.scale
            )
        else:
            return clip
    ...
    def apply_water_mask(
        self, image: ee.Image, remove_empty_pixels=False, apply_attributes: bool = True
    ):
        attributes = self.attributes if apply_attributes else self.attributes_clear
        for i, indice in enumerate(attributes):
            image = gee.apply_mask(
                image, self.water_mask, indice, indice + "_water", remove_empty_pixels
            )
        return image

    # extract image from collection
    def extract_image_from_collection(
        self,
        date,
        convolve: bool = False,
        convolve_radius: int = 1,
        ...
        try:
            collection = self.collection.filter(
                ee.Filter.date(
                    date.strftime("%Y-%m-%d"), (date + td(days=1)).strftime("%Y-%m-%d")
                )
            )
            ...
            if (convolve or self.convolve) and convolve_force_disabled == False:
                convolve_radius = (
                    convolve_radius
                    if self.convolve_radius is None or self.convolve_radius == 0
                    else self.convolve_radius
                )
    ...
    def split_geometry(self):
        ...
        if total > self.max_tile_pixels:
            ...
            latitudes = np.linspace(
                self.sample_lon_lat[0][1], self.sample_lon_lat[1][1], num=tiles + 1
            )
            longitudes = np.linspace(
                self.sample_lon_lat[0][0], self.sample_lon_lat[1][0], num=tiles + 1
            )
            ...
            for i, latitude in enumerate(latitudes[:-1]):
                for j, longitude in enumerate(longitudes[:-1]):
                    x1 = [i, j]
                    x2 = [i + 1, j + 1]
                    geometry = gee.get_geometry_from_lat_lon(
                        str(latitudes[x1[0]])
                        + ","
                        + str(longitudes[x1[1]])
                        + ","
    ...
    def apply_attributes(self, image: ee.Image, date):
        ...
        gldas = (
            ...
            .median()
            .resample("bicubic")
            .reproject(crs=image.projection(), scale=self.sensor_params["scale"])
        )
        ...
        for indice in self.indices_thresholds:
            if indice in df.columns:
                ...
                df_query = df.query(
                    ...
                    + str_op
                    + " "
                    + str(self.indices_thresholds[indice])
                )
                df.loc[df["index"].isin(df_query["index"]), "label"] = (
                    df_query["label"] + 1
                )
    ...
    def apply_grids(self, df: pd.DataFrame):
        ...
        # lat and lons range
        latitudes = np.linspace(
            self.sample_lon_lat[0][1], self.sample_lon_lat[1][1], num=width + 1
        )
        longitudes = np.linspace(
            self.sample_lon_lat[0][0], self.sample_lon_lat[1][0], num=height + 1
        )
        ...
        for i, lat in enumerate(latitudes):
            ...
            if i == 0:
                ...
                query = (
                    "(lat > " + str(lat) + " and lat <=" + str(latitudes[i + 1]) + ")"
                )
            if query:
        ...
        for i, lon in enumerate(longitudes):
            ...
            if i == 0:
                ...
                query = (
                    "(lon > " + str(lon) + " and lon <=" + str(longitudes[i + 1]) + ")"
                )
            if query:
    ...
    def apply_reducer(self, X_train, X_test, mode=1):
        ...
        if mode == 1:
            ...
            mlle = manifold.LocallyLinearEmbedding(
                n_neighbors=n_features_start,
                n_components=int(n_features_start * 0.75),
                method="modified",
                n_jobs=self.n_cores,
    ...
    def get_cache_files(self, date):
        prefix = (
            self.hash_string.encode()
            + self.lat_lon.encode()
            + self.sensor.encode()
            + str(self.morph_op).encode()
            + str(self.morph_op_iters).encode()
            + str(self.convolve).encode()
            + str(self.convolve_radius).encode()
            + str(self.shapefile_url).encode()
            + str(self.cloud_threshold).encode()
        )
        hash_image = hashlib.md5(
            prefix + (date.strftime("%Y-%m-%d") + "original").encode()
        )
        hash_timeseries = hashlib.md5(
            prefix
            + (
                self.dates_timeseries[0].strftime("%Y-%m-%d")
                + self.dates_timeseries[1].strftime("%Y-%m-%d")
            ).encode()
            + str(self.days_in).encode()
            + str(self.days_out).encode()
            + str(self.normalized).encode()
            + str(self.fill_missing).encode()
            + str(self.reducer).encode()
            + str(self.class_mode).encode()
        )
        hash_classifiers = hashlib.md5(
            prefix
            + (
                self.dates_timeseries[0].strftime("%Y-%m-%d")
                + self.dates_timeseries[1].strftime("%Y-%m-%d")
        ...
        return [
            self.cache_path + "/" + hash_image.hexdigest(),
            self.cache_path + "/" + hash_timeseries.hexdigest(),
            self.cache_path + "/" + hash_classifiers.hexdigest(),
            self.cache_path + "/" + hash_runtime.hexdigest(),
        ]

    # fill missing dates in a dataframe
    def fill_missing_dates(
        self, df: pd.DataFrame, min_date=None, max_date=None, fill: str = "dummy"
    ):
        ...
        if self.pixels_lat_lons is None:
            pixels_lat_lons = np.unique(
                df[["pixel", "row", "column", "lat", "lon"]].values.astype(str), axis=0
            )
        else:
        ...
        df_diff = df2.merge(df1, how="outer", indicator=True).loc[
            lambda x: x["_merge"] == "right_only"
        ][[0, 1, 2, 3, 4, 5]]
        df_diff = df_diff.rename(
            columns={0: "pixel", 1: "row", 2: "column", 3: "lat", 4: "lon", 5: "date"}
        )
        ...
        if fill == "dummy":
            ...
            df = (
                ...
                .apply(
                    ...
                    .set_index("date")
                    .interpolate(method=fill)
                )
                .drop(["pixel", "level_0"], axis=1)
                .reset_index()
                .sort_values(by=["pixel", "date"])
            )
    ...
    def fix_timeseries(self, df: pd.DataFrame):
        ...
        # fix other columns
        df[["lat", "lon"]] = df[["lat", "lon"]].astype(
            dtype=np.float64, errors="ignore"
        )
        df["date"] = pd.to_datetime(df["date"], errors="ignore")
        df["doy"] = pd.to_datetime(
            df["date"], format="%Y-%m-%d", errors="ignore"
        ).dt.month.astype(dtype=np.int64, errors="ignore")
    ...
    def extract_image_pixels(
        ...
        try:
            ...
            if lons_lats_attributes is None or len(lons_lats_attributes) != (
                len(self.df_columns if apply_attributes else self.df_columns_clear) - 6
            ):
                raise Exception()
        ...
        # error finding image in cache
            ...
            try:
                ...
                if apply_attributes:
                    lons_lats_attributes = np.array([]).reshape(
                        0, len(self.attributes) + 2
                    )
                else:
                    lons_lats_attributes = np.array([]).reshape(
                        0, len(self.attributes_clear) + 2
                    )
                for i, geometry in enumerate(self.splitted_geometry):
                    ...
                    print(
                        ...
                        + str(i + 1)
                        + " of "
                        + str(len(self.splitted_geometry))
                        + "..."
                    )

                    # counters
                    clip = self.clip_image(
                        self.extract_image_from_collection(
                            date=date, apply_attributes=apply_attributes
                        ),
                        geometry=geometry,
                        scale=self.scale,
                    )
                    if not clip is None:
                        counters = (
                            clip.select(
                                [self.sensor_params["red"], "water", "water_nocloud"]
                            )
                            .reduceRegion(
                        ...
                        if len(self.splitted_geometry) > 1:
                            sample_total_pixel = gee.get_image_counters(
                                image=self.clip_image(ee.Image(abs(self.dummy))).select(
                                    "constant"
                                ),
                                geometry=geometry,
                                scale=self.scale,
                        ...
                        pixel_score = (
                            0.0
                            if total_pixel == 0
                            else round(total_pixel / sample_total_pixel, 5)
                        )
                        water_nocloud_score = (
                            0.0
                            if total_water_nocloud == 0
                            else round(total_water_nocloud / total_water_pixel, 5)
                        )
                        ...
                        if (
                            pixel_score >= 0.50
                            and water_nocloud_score >= self.cloud_threshold
                        ) or disable_threshold:
                            if apply_attributes:
                                geometry_lons_lats_attributes = (
                                    gee.extract_latitude_longitude_pixel(
                                        image=clip,
                                        geometry=self.geometry,
                                        bands=[a + "_water" for a in self.attributes],
                                        scale=self.sensor_params["scale"],
                                    )
                                )
                            else:
                                geometry_lons_lats_attributes = (
                                    gee.extract_latitude_longitude_pixel(
                                        ...
                                        geometry=self.geometry,
                                        bands=[
                                            a + "_water" for a in self.attributes_clear
                                        ],
                                        scale=self.sensor_params["scale"],
                                    )
                                )
                            lons_lats_attributes = np.vstack(
                                (lons_lats_attributes, geometry_lons_lats_attributes)
                            )
        ...
        try:
            ...
            # build arrays
            extra_attributes = np.array(
                list(
                    zip(
                        [0] * len(lons_lats_attributes),
                        [0] * len(lons_lats_attributes),
                        [0] * len(lons_lats_attributes),
                        [0] * len(lons_lats_attributes),
                        [date.strftime("%Y-%m-%d")] * len(lons_lats_attributes),
                        [0] * len(lons_lats_attributes),
                    )
                )
            )
            ...
            df_timeseries = (
                pd.DataFrame(
                    data=lons_lats_attributes,
                    columns=self.df_columns
                    if apply_attributes
                    else self.df_columns_clear,
                )
                .infer_objects()
                .sort_values(["lat", "lon"])
    ...
    def process_timeseries_data(self, force_cache: bool = False):
        ...
        df_timeseries.drop_duplicates(
            subset=["pixel", "date", "lat", "lon"] + self.attributes,
            keep="last",
            inplace=True,
        )
        ...
        self.limits = [
            math.ceil(self.resolution[0] / self.plots_grid),
            math.ceil(self.resolution[1] / self.plots_grid),
        ]

        # save modified dataframe to its original variable and apply grids
        self.df_timeseries = self.apply_grids(
            df=df_timeseries[self.df_columns].dropna()
        )
        ...
        self.dates_timeseries_interval = [
            dt.strptime(date.astype(str).split("T")[0], "%Y-%m-%d")
            for date in self.df_timeseries["date"].unique()
        ]
    ...
    def process_training_data(self, df: pd.DataFrame):
        ...
        # fix dataframes
        self.pixels_lat_lons = np.unique(
            df[["pixel", "row", "column", "lat", "lon"]].values.astype(str), axis=0
        )
        ...
        self.classification_dates = [
            (self.dates_timeseries_interval[-1] - td(days=int(d)))
            for d in range(0, self.days_out)
        ]
        ...
        self.predict_dates = [
            (self.classification_dates[-1] + td(days=int(d)))
            for d in range(1, len(self.classification_dates) + 1)
        ]
        ...
        df_pixel = misc.series_to_supervised(
            df[self.attributes_selected + ["label"]].values,
            self.days_in,
            self.days_out,
            dropnan=True,
        )
        ...
        in_labels = [
            s
            for i, s in enumerate(df_pixel.columns)
            if "t-" in s and not str_label + "(" in s
        ]
        in_labels = [
            s
            for i, s in enumerate(in_labels)
            if (self.attribute_lat_lon or not str_lat in s)
            and (self.attribute_lat_lon or not str_lon in s)
            and (self.attribute_doy or not str_doy in s)
        ]
        if self.class_mode:
            out_labels = [
                s
                for i, s in enumerate(df_pixel.columns)
                if str_label + "(" in s and not "t-" in s
            ]
        else:
            out_labels = [
                s
                for i, s in enumerate(df_pixel.columns)
                if (
                    str_label_mndwi + "(" in s
                    or str_label_ndvi + "(" in s
                    or str_label_sabi + "(" in s
                    or str_label_fai + "(" in s
                )
                and not "t-" in s
            ]
        ...
        X = df_pixel[df_pixel.columns[: -len(out_labels)]].values.reshape(
            (-1, len(df_pixel.columns[: -len(out_labels)]))
        )
        y = df_pixel[df_pixel.columns[-len(out_labels) :]].values.reshape(
            (-1, len(df_pixel.columns[-len(out_labels) :]))
        )
        ...
        print(
            ...
            % (
                ...
                self.df_train[1].shape,
                self.df_test[0].shape,
    ...
    def train(self, disable_gpu: bool = True, batch_size: int = 2048):
        ...
        self.rs_train_size = (
            int(self.rs_train_size)
            if self.rs_train_size > 1
            else float(self.rs_train_size)
            if self.rs_train_size <= len(self.df_train)
            else len(self.df_train)
        )

        # get data
        X_train, y_train = self.df_train
        X_test, y_test = self.df_test
        X_gridsearch, _, y_gridsearch, _ = model_selection.train_test_split(
            X_train,
            y_train,
            train_size=self.rs_train_size,
            random_state=self.random_state,
                ...
                class_weight2 = dict(
                    zip(
                        ...
                        utils.class_weight.compute_class_weight(
                            "balanced", classes, self.apply_label_y(y_gridsearch)
                        ),
                    )
                )
        ...
        if self.model is None or self.model == "mlp":
            try:
                ...
                # change label size
                if self.class_mode:
                    y_gridsearch_categorical = tf.keras.utils.to_categorical(
                        y_gridsearch
                    )
                    y_gridsearch_ = y_gridsearch_categorical.reshape(
                        y_gridsearch_categorical.shape[0],
                        y_gridsearch_categorical.shape[1]
                        * y_gridsearch_categorical.shape[2],
                    )
                    y_train_categorical = tf.keras.utils.to_categorical(y_train)
                    y_train_ = y_train_categorical.reshape(
                        y_train_categorical.shape[0],
                        y_train_categorical.shape[1] * y_train_categorical.shape[2],
                    )
                ...
                class KerasRegressorModified(
                    tf.keras.wrappers.scikit_learn.KerasRegressor
                ):
                    def fit(self, x, y, **kwargs):
                        return super(
                            tf.keras.wrappers.scikit_learn.KerasRegressor, self
                        ).fit(
                            x,
                            y,
                        ...
                        mlp_modified = tf.keras.Sequential(
                            [
                                tf.keras.layers.Dense(
                                    512, activation="tanh", input_shape=(in_attributes,)
                                ),
                                ...
                                tf.keras.layers.Dropout(dropout),
                                tf.keras.layers.Dense(
                                    y_gridsearch_.shape[1], activation="softmax"
                                ),
                            ]
                        )
                ...
                rs = model_selection.RandomizedSearchCV(
                    estimator=KerasRegressorModified(build_fn=mlp_modified, verbose=1),
                ...
                str_model = (
                    "MLP (dropout="
                    + str(rs.best_params_["dropout"])
                    + ",epochs="
                    + str(rs.best_params_["epochs"])
                    + ",optimizer="
                    + str(rs.best_params_["optimizer"])
                    + ")"
                )
                ...
                measures = misc.concordance_measures(
                    metrics.confusion_matrix(y_test_label, y_pred_label),
                    y_test_label,
                    y_pred_label,
                )
            ...
            try:
                ...
                class KerasRegressorModified(
                    ...
                    def fit(self, x, y, **kwargs):
                        kwargs = self.filter_sk_params(
                            tf.keras.Sequential.predict_classes, kwargs
                        )
                        sample_weight = (
                            np.array(
                                [
                        ...
                        [
                            tf.keras.layers.Bidirectional(
                                tf.keras.layers.LSTM(
                                    int(size),
                                    activation="tanh",
                                    input_shape=(1, X_gridsearch.shape[1]),
                                )
                            ),
                            tf.keras.layers.Dropout(dropout),
                            ...
                            tf.keras.layers.Bidirectional(
                                tf.keras.layers.LSTM(
                                    int(size), activation="tanh", return_sequences=True
                                )
                            ),
                            tf.keras.layers.Dropout(dropout),
                            tf.keras.layers.TimeDistributed(
                                tf.keras.layers.Dense(int(size), activation="tanh")
                            ),
                            tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(1)),
                        ]
                    )
                    lstm_modified.compile(
                        optimizer=optimizer, loss="mse", metrics=["mae"]
                    )
                ...
                def lstm_scorer(y_true, y_pred):
                    return -metrics.mean_squared_error(
                        y_true.reshape(1, -1)[0], y_pred.reshape(1, -1)[0]
                    )
                ...
                start_time = time.time()
                rs = model_selection.RandomizedSearchCV(
                    estimator=KerasRegressorModified(build_fn=lstm_modified, verbose=1),
                    param_distributions=random_grid,
                    scoring=metrics.make_scorer(lstm_scorer),
                    n_iter=self.rs_iter,
                    cv=5,
                    verbose=1,
                    random_state=self.random_state,
                ...
                rs.fit(
                    X_gridsearch.reshape(
                        X_gridsearch.shape[0], 1, X_gridsearch.shape[1]
                    ),
                    y_gridsearch.reshape(
                        y_gridsearch.shape[0], y_gridsearch.shape[1], 1
                    ),
                )
                lstm = rs.best_estimator_
                lstm.fit(
                    X_train.reshape(X_train.shape[0], 1, X_train.shape[1]),
                    y_train.reshape(y_train.shape[0], y_gridsearch.shape[1], 1),
                )
        ...
        if self.model is None or self.model == "rf":
            try:
                ...
                str_model = (
                    "RF (n_estimators="
                    + str(rs.best_params_["n_estimators"])
                    + ",max_features="
                    + str(rs.best_params_["max_features"])
                    + ",max_depth="
                    + str(rs.best_params_["max_depth"])
                    + ",min_samples_leaf="
                    + str(rs.best_params_["min_samples_leaf"])
                    + ",min_samples_split="
                    + str(rs.best_params_["min_samples_split"])
                    + ",bootstrap="
                    + str(rs.best_params_["bootstrap"])
                    + ")"
                )
                self.classifiers_runtime[str_model] = time.time() - start_time
        ...
        if self.model is None or self.model == "svm":
            try:
                ...
                random_grid = [
                    {
                        ...
                        "estimator__gamma": scipy.stats.expon(scale=0.100),
                ...
                if self.class_mode:
                    ...
                    rs = model_selection.RandomizedSearchCV(
                        estimator=multioutput.MultiOutputClassifier(
                            svm.SVC(
                                verbose=0,
                                random_state=self.random_state,
                                class_weight=class_weight2,
                        ...
                        param_distributions=random_grid_clear,
                        scoring="neg_mean_squared_error",
                        n_iter=self.rs_iter,
                        cv=5,
                        verbose=1,
                    ...
                    rs = model_selection.RandomizedSearchCV(
                        estimator=multioutput.MultiOutputRegressor(
                            svm.SVR(verbose=0), n_jobs=self.n_cores
                        ),
                        param_distributions=random_grid,
                        scoring="neg_mean_squared_error",
                        n_iter=self.rs_iter,
                ...
                if self.class_mode:
                    str_model = (
                        "SVM (kernel="
                        + str(rs.best_params_["estimator__kernel"])
                        + ",C="
                        + str(rs.best_params_["estimator__C"])
                        + ",gamma="
                        + str(rs.best_params_["estimator__gamma"])
                        + ",shrinking="
                        + str(rs.best_params_["estimator__shrinking"])
                        + ")"
                    )
                else:
                    str_model = (
                        ...
                        + str(rs.best_params_["estimator__gamma"])
                        + ",epsilon="
                        + str(rs.best_params_["estimator__epsilon"])
                        + ",shrinking="
                        + str(rs.best_params_["estimator__shrinking"])
                        + ")"
                    )
    ...
    def predict(self, folder: str, path: str = None, chla_threshold: int = 20):
        ...
        collection, _ = gee.get_sensor_collections(
            geometry=self.geometry,
            sensor=self.sensor,
            dates=[
                dt.strftime(self.dates_timeseries[0], "%Y-%m-%d"),
                dt.strftime(self.predict_dates[-1] + td(days=1), "%Y-%m-%d"),
            ],
        )
        self.collection = collection
        ...
        for date in self.predict_dates:
            # extract pixels from image
            # check if is good image (with pixels)
            df_predict_ = self.extract_image_pixels(
                image=self.extract_image_from_collection(
                    date=date, apply_attributes=False, convolve_force_disabled=True
                ),
                date=date,
        ...
        try:
            for date in self.predict_dates:
                ...
                # check if is good image (with pixels)
                image = (
                    ee.Image(abs(self.dummy))
                    .blend(
                        ee.Image.load(path + "/" + date.strftime("%Y-%m-%d") + "_chla")
                    )
                    .rename("chla")
                )
                image = gee.apply_mask(
                    image, self.water_mask, "chla", "chla_water", False
                )

                # go through each geometry and get pixels
                for geometry in self.splitted_geometry:
                    clip = self.clip_image(
                        image=image, geometry=geometry, scale=self.scale
                    )
                    geometry_lons_lats_attributes = (
                        gee.extract_latitude_longitude_pixel(
                            image=clip,
                ...
                extra_attributes = np.array(
                    list(zip(range(0, len(lons_lats_attributes))))
                )
                extra_attributes2 = np.array(
                    list(zip([date] * len(lons_lats_attributes)))
                )
                lons_lats_attributes = np.hstack(
                    (extra_attributes, extra_attributes2, lons_lats_attributes)
                )

                # append new data from roi to dataframe
                df_reference_ = pd.DataFrame(
                    data=lons_lats_attributes, columns=df_columns_reference
                ).infer_objects()
            ...
            df_reference.loc[
                (
                    (df_reference["chla"] >= 0)
                    & (df_reference["chla"] <= chla_threshold)
                ),
                "label",
            ] = 0
            df_reference.loc[(df_reference["chla"] > chla_threshold), "label"] = len(
                attributes_clear
            )
        ...
        for attribute in attributes_clear:
            df_classification = df_classification[
                (df_classification[attribute] != abs(self.dummy))
            ]
        ...
        df_classification.loc[
            (df_classification["cloud"] == abs(self.dummy)), "cloud"
        ] = 0.0
        ...
        df_predict = self.apply_label(
            self.fill_missing_dates(
                df=df_predict,
                min_date=min(self.predict_dates),
                max_date=max(self.predict_dates),
            )
        ).reset_index(drop=True)

        # fix values in dataframe - classification
        # concat it with timeseries dataframe before doing the
        # concatenation and remove the old dates after that
        df_classification = self.fill_missing_dates(
            df=pd.concat([self.df_timeseries, df_classification]),
            min_date=min(self.classification_dates),
            max_date=max(self.classification_dates),
            fill=self.fill_missing,
        ).reset_index(drop=True)
        df_classification = df_classification[
            df_classification["date"] >= min(self.classification_dates)
        ]
        ...
        df_classification_proc = misc.series_to_supervised(
            df_classification[
                df_classification["doy"] == max(df_classification["doy"])
            ][self.attributes_selected].values,
            self.days_in,
            self.days_out,
            dropnan=True,
        )
        str_lat = (
            "var"
            + str(self.attributes_selected.index("lat") + 1)
            + "(t-"
            + str(self.days_in)
            + ")"
        )
        str_lon = (
            "var"
            + str(self.attributes_selected.index("lon") + 1)
            + "(t-"
            + str(self.days_in)
            + ")"
        )
        in_labels = [
            s for i, s in enumerate(df_classification_proc.columns) if "t-" in s
        ]
        in_labels = [
            s
            for i, s in enumerate(in_labels)
            if (
                self.attribute_lat_lon
                or not "var" + str(self.attributes_selected.index("lat") + 1) in s
            )
            and (
                self.attribute_lat_lon
                or not "var" + str(self.attributes_selected.index("lon") + 1) in s
            )
            and (
                self.attribute_doy
                or not "var" + str(self.attributes_selected.index("doy") + 1) in s
            )
        ]
        ...
        X = self.scaler.transform(
            df_classification_proc[in_labels].values.reshape((-1, len(in_labels)))
        )
        if self.reducer:
        ...
        for index, row in df_predict.iterrows():
            features.append(
                geojson.Feature(
                    geometry=geojson.Point((float(row["lat"]), float(row["lon"]))),
                    properties={
                        "index": int(index),
        ...
        # plot configuration
        image_empty_clip_io = PIL.Image.open(
            BytesIO(
                requests.get(
                    self.clip_image(ee.Image([99999, 99999, 99999]))
                    .select(["constant", "constant_1", "constant_2"])
                    .getThumbUrl({"min": 0, "max": 99999}),
                    timeout=60,
                ).content
            )
        )
        markersize_scatter = (72.0 / 300) * ((self.scale / self.resolution[0]) * 4)
        xticks = np.linspace(
            self.sample_lon_lat[0][1],
            self.sample_lon_lat[1][1],
            num=self.plots_grid + 1,
        )
        yticks = np.linspace(
            self.sample_lon_lat[0][0],
            self.sample_lon_lat[1][0],
            num=self.plots_grid + 1,
        )
        ...
        colorbar_ticks = np.linspace(
            0, colorbar_ticks_max if colorbar_ticks_max > 1 else 2, num=11, dtype=int
        )
        ...
        legends_colors = [
            Rectangle(
                (0, 0), 1, 1, linewidth=0, edgecolor=None, facecolor="cyan", fill=True
            ),
            Rectangle(
                (0, 0),
                1,
            ...
            Rectangle(
                (0, 0), 1, 1, linewidth=0, edgecolor=None, facecolor="gray", fill=True
            ),
            Rectangle(
                (0, 0), 1, 1, linewidth=0, edgecolor=None, facecolor="black", fill=True
            ),
        ]
        ...
        for model in self.classifiers:
            ...
            if "MLP" in model:
                ...
                if self.class_mode:
                    ...
                    y_pred = y_pred.reshape(
                        y_pred.shape[0],
                        self.randomizedsearch_categorical[1],
                        self.randomizedsearch_categorical[2],
                    )
                    y_pred = np.argmax(y_pred, axis=1)
            ...
            # OR LSTM
                ...
                if self.class_mode:
                    y_pred = (
                        ...
                        .predict(X.reshape(X.shape[0], 1, X.shape[1]))
                        .reshape(-1, self.days_out * 1)
                    )
                else:
                    y_pred = (
                        self.classifiers[model]
                        .predict(X.reshape(X.shape[0], 1, X.shape[1]))
                        .reshape(-1, self.days_out * len(attributes_clear))
                    )
            ...
            if self.propagate and self.days_out > 1:
                for i in range(1, self.days_out):
                    ...
                    if self.class_mode:
                        features = 1
                        df_new = df.rename(columns={0: "lat", 1: "lon", 2: "label"})[
                            ["lat", "lon", "label"]
                        ]
                    else:
                        df_new = df.rename(columns=dict_attributes)[
                            ["lat", "lon"] + attributes_clear
                        ]
                    ...
                    df_classification = (
                        df_classification[
                            df_classification["date"] != df_classification["date"].min()
                        ]
                        .append(df_new)
                        .fillna(method="ffill")
                    )
                    df_classification = self.fill_missing_dates(
                        ...
                        min_date=df_classification["date"].min(),
                        max_date=df_classification["date"].max(),
                        fill=self.fill_missing,
                    ).reset_index(drop=True)
                    df_classification_proc = misc.series_to_supervised(
                        df_classification[
                    ...
                    X = self.scaler.transform(
                        df_classification_proc[in_labels].values.reshape(
                            (-1, len(in_labels))
                        )
                    )
                    if self.reducer:
                    ...
                    if "MLP" in model:
                        ...
                        if self.class_mode:
                            ...
                            y_pred = np.argmax(y_pred, axis=1)
                            df = pd.DataFrame(
                                data=np.concatenate((array_lats_lons, y_pred), axis=1)
                            )
                        else:
                            df = pd.DataFrame(
                                data=np.concatenate((array_lats_lons, y_pred), axis=1)
                    ...
                    # regular dataset format (rf and svm)
                        ...
                        df_new = pd.DataFrame(
                            data=np.concatenate(
                                (array_lats_lons, self.classifiers[model].predict(X)),
                                axis=1,
                            )
                        )

                    # append new predictions to incorporate and propagate
                    # it with previous ones
                    df_new.columns = df_new.columns.astype(int)
                    df_new2 = pd.DataFrame(
                        data=df_new[df_new.columns.values[2 : -(features * i)]].values,
                        columns=df_new.columns.values[2 : -(features * i)].astype(int)
                        + (features * i),
                    )
                    ...
                    for c in range(2, (features * i) + 2):
                        df_new2[c] = np.nan
                    df = (
                        df.append(df_new2.sort_index(axis=1))
                        .groupby([0, 1])
                        .median()
                        .reset_index()
                        .fillna(method="ffill")
            ...
            # save time
            self.classifiers_runtime[model] = self.classifiers_runtime[model] + (
            ...
            for i, date in enumerate(self.predict_dates):
                ...
                if self.class_mode:
                    ...
                    df_pred.rename(
                        columns={0: "lat", 1: "lon", i + 2: "label"}, inplace=True
                    )
                else:
                ...
                df_pred.rename(
                    columns={"label": "label_predicted_" + str(i)}, inplace=True
                )
                if not model in self.predictions:
                    self.predictions[model] = df_pred
                else:
                    self.predictions[model] = self.predictions[model].join(
                        df_pred.drop(["lat", "lon", "index"], axis=1)
                    )
            c_model += 1
            ...
            for i, date in enumerate(self.predict_dates):
                df_prediction_ = (
                    self.predictions[model][["lat", "lon", "label_predicted_" + str(i)]]
                    .copy()
                    .rename(columns={"label_predicted_" + str(i): "label_predicted"})
                )
            ...
            for index, row in df_true.iterrows():
                df_prediction.loc[
                    (df_prediction["date"] == row["date"])
                    & (df_prediction["lat"] == row["lat"])
                    & (df_prediction["lon"] == row["lon"]),
                    "pixel",
                ] = row["pixel"]
                df_prediction.loc[
                    (df_prediction["date"] == row["date"])
            ...
            # fix indexes
            df_prediction.loc[
                (df_prediction["label"].isnull()), "label"
            ] = df_prediction[df_prediction["label"].isnull()]["label_predicted"]
            df_prediction.reset_index(inplace=True, drop=True)

            # fix indetermined pixels
            df_prediction.loc[
                (df_prediction["label_predicted"] < len(attributes_clear)),
                "label_predicted",
            ] = 0
            ...
            for i, date in enumerate(self.predict_dates):
                ...
                for i, row in df_prediction_.iterrows():
                    ...
                    # get pixels
                    df_grid = df_prediction_[
                        (
                            (df_prediction_["column"] >= col_start)
                            & (df_prediction_["column"] <= col_end)
                        )
                        & (
                            (df_prediction_["row"] >= row_start)
                            & (df_prediction_["row"] <= row_end)
                        )
                    ]
                    if len(df_grid) > 0:
                        ...
                        # true
                        df_grid.loc[
                            df_grid["label"] < len(attributes_clear), "label"
                        ] = 0
                        df_grid.loc[
                            df_grid["label"] == len(attributes_clear), "label"
                        ] = 1
                        pct_occurrence = int(
                            (df_grid["label"].sum() / len(df_grid)) * 100
                        )
                        pct_occurrence = pct_occurrence if pct_occurrence > 0 else 0
                        df_prediction.loc[
                            (df_prediction["index"].isin(df_grid["index"].values)),
                            "class",
                        ] = pct_occurrence

                        # prediction
                        df_grid.loc[
                            df_grid["label_predicted"] < len(attributes_clear),
                            "label_predicted",
                        ] = 0
                        df_grid.loc[
                            df_grid["label_predicted"] == len(attributes_clear),
                        ...
                        pct_occurrence = int(
                            (df_grid["label_predicted"].sum() / len(df_grid)) * 100
                        )
                        pct_occurrence = pct_occurrence if pct_occurrence > 0 else 0
                        df_prediction.loc[
                            (df_prediction["index"].isin(df_grid["index"].values)),
            ...
            for index, color in enumerate(color_map):
                ...
                df_prediction.loc[
                    (df_prediction["label_predicted"] == index), "color_predicted"
                ] = color[0]
                        ...
                        features.append(
                            ee.Feature(
                                ee.Geometry.Point(row["lat"], row["lon"]),
                                {
                                    ...
                                    "class": int(row["class"]),
                                    "class_predicted": int(row["class_predicted"]),
                                },
                            )
                        )
            ...
            df_prediction.to_csv(
                folder + "/csv/df_prediction_" + str(model_short) + ".csv"
            )
            ...
            if not model in self.merges:
                ...
                self.merges[model] = self.merges[model].append(
                    df_prediction, ignore_index=True
                )
            ...
            for plot_type in plot_types:
                ...
                if plot_type == "pixel":
                    plt.title(
                        self.name
                        + ": pixel-wise from "
                        + self.predict_dates[0].strftime("%Y-%m-%d")
                        + " to "
                        + self.predict_dates[-1].strftime("%Y-%m-%d")
                        + "\n"
                        + model,
                        fontdict={"fontsize": 8},
                        pad=30,
                ...
                for i, date in enumerate(self.predict_dates):
                    ...
                    # extract image
                    image = self.extract_image_from_collection(
                        date=date, apply_attributes=False
                    )
                    if image:
                        image_clip = self.clip_image(image)
                        image_clip_rgb = image_clip.select(
                            self.sensor_params["red"],
                            self.sensor_params["green"],
                            self.sensor_params["blue"],
                        ).getThumbUrl({"min": 0, "max": 3000})
                        image_clip_io = PIL.Image.open(
                            BytesIO(requests.get(image_clip_rgb, timeout=60).content)
                        )
                    else:
                    ...
                    if plot_type != "scene":
                        ...
                        c.set_title(
                            date.strftime("%Y-%m-%d"), fontdict={"fontsize": 4.5}
                        )
                        c.set_xticks(xticks)
                        c.set_yticks(yticks)
                ...
                if plot_type == "pixel":
                    for i, date in enumerate(self.predict_dates):
                        ...
                        if len(df_prediction_) > 0:
                            c.scatter(
                                df_prediction_[(df_prediction_["cloud"] == 0)]["lat"],
                                df_prediction_[(df_prediction_["cloud"] == 0)]["lon"],
                                marker="s",
                                s=markersize_scatter,
                                c=df_prediction_[(df_prediction_["cloud"] == 0)][
                                    "color"
                                ].values,
                                edgecolors="none",
                            )
                            c.scatter(
                                df_prediction_[(df_prediction_["cloud"] == 1)]["lat"],
                                df_prediction_[(df_prediction_["cloud"] == 1)]["lon"],
                                marker="s",
                                s=markersize_scatter,
                                c="gray",
                                edgecolors="none",
                ...
                # grid
                    ...
                    for i, date in enumerate(self.predict_dates):
                        ...
                        if len(df_prediction_) > 0:
                            ...
                            c.scatter(
                                df_prediction_[
                                    (df_prediction_["cloud"] == 0)
                                    & (df_prediction_["label"] == -1)
                                ]["lat"],
                                df_prediction_[
                                    (df_prediction_["cloud"] == 0)
                                    & (df_prediction_["label"] == -1)
                ...
                for i, date in enumerate(self.predict_dates):
                    ...
                    if plot_type == "pixel":
                        ...
                        # labels arrays
                        df_merge_ = df_prediction_[
                            (df_prediction_["cloud"] == 0)
                            & (
                                (df_prediction_["label"] == 0)
                                | (df_prediction_["label"] == len(attributes_clear))
                            )
                        ]
                        if len(df_merge_) > 0:
                            y_pred = df_merge_["label_predicted"].values.reshape(
                                (-1, 1)
                            )
                        ...
                        dict_results.append(
                            {
                                ...
                                "size_train": str(
                                    "D="
                                    + str(self.df_train[0].shape)
                                    + "Dy="
                                    + str(self.df_train[0].shape)
                                    + ";R="
                                    + str(self.df_randomizedsearch[0].shape)
                                    + ";Ry="
                                    + str(self.df_randomizedsearch[1].shape)
                                ),
                        ...
                        title_plot = (
                            "Pred. (Acc:"
                            + str(round(measures["acc"], 2))
                            + ",Kpp:"
                            + str(round(measures["kappa"], 2))
                            + ",MCC:"
                            + str(round(measures["mcc"], 2))
                            + ",F1:"
                            + str(round(measures["f1score"], 2))
                            + ")"
                        )
                        ...
                        if len(df_prediction_) > 0:
                            c.scatter(
                                ...
                                marker="s",
                                s=markersize_scatter,
                                c=df_prediction_["color_predicted"].values,
                                edgecolors="none",
                            )
                    ...
                    # grid
                        ...
                        title_plot = (
                            "Pred. (RMSE:"
                            + str(round(measures["rmse"], 2))
                            + ",MAE:"
                            + str(round(measures["mae"], 2))
                            + ",R^2:"
                            + str(round(measures["r2score"], 2))
                            + ")"
                        )
                        ...
                        if len(df_prediction_) > 0:
                            s = c.scatter(
                                ...
                                marker="s",
                                s=markersize_scatter,
                                c=df_prediction_["class_predicted"],
                                cmap=plt.get_cmap("jet"),
                                edgecolors="none",
                            )
                            s.set_clim(colorbar_ticks[0], colorbar_ticks[-1])
                    ...
                    # scene
                        ...
                        # fix label value
                        df_prediction_.loc[
                            (df_prediction_["label"] < len(attributes_clear)), "label"
                        ] = 0
                        df_prediction_.loc[
                            (df_prediction_["label"] == len(attributes_clear)), "label"
                        ] = 1
                        df_prediction_.loc[
                            (df_prediction_["label_predicted"] < len(attributes_clear)),
                            "label_predicted",
                        ] = 0
                        df_prediction_.loc[
                            (
                                df_prediction_["label_predicted"]
                                == len(attributes_clear)
                            ),
                            "label_predicted",
                        ] = 1
                        ...
                        if len(df_prediction_) > 0:
                            ...
                            # true
                            pct_true = int(
                                (df_prediction_["label"].sum() / len(df_prediction_))
                                * 100
                            )
                            ...
                            pct_pred = int(
                                (
                                    df_prediction_["label_predicted"].sum()
                                    / len(df_prediction_)
                                )
                                * 100
                            )
                ...
                if plot_type == "scene":
                    ...
                    ax = self.df_scene[["validation", "prediction"]].plot(
                        kind="bar", width=0.5, ax=ax_
                    )
                    ax2 = self.df_scene["difference"].plot(
                        secondary_y=True, color="red", ax=ax_
                    )
                    ...
                    ax.set_ylim(
                        self.df_scene["difference"].min(),
                        self.df_scene["validation"].max() + 1
                        if self.df_scene["validation"].max()
                        > self.df_scene["prediction"].max()
                        else self.df_scene["prediction"].max() + 1,
                    )
                    ...
                    plt.legend(
                        handles=handles,
                        labels=["Validation/Reference", "Prediction", "Difference"],
                        loc="upper right",
                        fancybox=True,
                        shadow=True,
                    )
                ...
                # others
                    ...
                    if plot_type == "grid":
                        ...
                        cbar = fig.colorbar(
                            images_grid[-1],
                            cax=fig.add_axes([0.601, 0.055, 0.25, 0.01]),
                        ...
                        plt.legend(
                            legends_colors2,
                            legends_colors_captions2,
                            loc="upper center",
                            bbox_to_anchor=(-1.30, 0.4),
                            ncol=4,
                            fontsize="x-small",
                            fancybox=True,
                            shadow=True,
    ...
    def predict_reduction(self, folder: str, reduction: str = "median"):
        ...
        markersize_scatter = (72.0 / 300) * ((self.scale / self.resolution[0]) * 8.5)
        xticks = np.linspace(
            self.sample_lon_lat[0][1],
            self.sample_lon_lat[1][1],
            num=self.plots_grid + 1,
        ...
        if not self.merges is None and len(self.merges) > 0:
            ...
            for model in self.merges:
                ...
                # fix merges
                self.merges[model] = self.merges[model].apply(
                ...
                df_reduction.loc[
                    (df_reduction["label"] < len(attributes_clear)), "label"
                ] = 0
                df_reduction.loc[
                    (df_reduction["label"] == len(attributes_clear)), "label"
                ] = 1
                df_reduction.loc[
                    (df_reduction["label_predicted"] < len(attributes_clear)),
                    "label_predicted",
                ] = 0
                df_reduction.loc[
                    (df_reduction["label_predicted"] == len(attributes_clear)),
                ...
                if reduction == "min":
                    df_reduction = (
                        df_reduction.groupby(["lat", "lon"]).min().reset_index()
                    )
                else:
                    df_reduction = (
                        df_reduction.groupby(["lat", "lon"]).median().reset_index()
                    )
                df_scene_median = (
                    self.df_scene[self.df_scene["model"] == model_short]
                    .groupby(["model"])
                    .median()
                    .reset_index()
                )
                ...
                df_reduction.loc[
                    (df_reduction["label_predicted"] > 0.5), "label_predicted"
                ] = 1
                df_reduction.loc[
                    (df_reduction["label_predicted"] <= 0.5), "label_predicted"
                ] = 0
                ...
                for index, color in enumerate(color_map):
                    df_reduction.loc[(df_reduction["label"] == index), "color"] = color[
                        0
                    ]
                    df_reduction.loc[
                        (df_reduction["label_predicted"] == index), "color_predicted"
                    ] = color[0]
                ...
                if len(self.predict_dates) > 1:
                    plt.title(
                        self.name
                        + ": pixel/grid/scene-wise reductions from "
                        + self.predict_dates[0].strftime("%Y-%m-%d")
                        + " to "
                        + self.predict_dates[-1].strftime("%Y-%m-%d")
                        + "\n"
                ...
                c.set_title(
                    "Validation/Reference (" + str(reduction) + ")",
                    fontdict={"fontsize": 8},
                )
                c.set_xticks(xticks)
                c.set_yticks(yticks)
                ...
                cbar = fig.colorbar(
                    s,
                    cax=fig.add_axes([0.635, 0.64, 0.18, 0.01]),
                    ticks=colorbar_ticks,
                    orientation="horizontal",
                )
                ...
                c.scatter(
                    ...
                    marker="s",
                    s=markersize_scatter,
                    c=df_reduction["color_predicted"].values,
                    edgecolors="none",
                )
                c.margins(x=0, y=0)
                ...
                s = c.scatter(
                    ...
                    marker="s",
                    s=markersize_scatter,
                    c=df_reduction["class_predicted"],
                    cmap=plt.get_cmap("jet"),
                    edgecolors="none",
                )
                s.set_clim(colorbar_ticks[0], colorbar_ticks[-1])
                ...
                # save dataframe
                df_reduction.to_csv(
                    folder + "/csv/df_prediction_" + str(model_short) + "_median.csv"
                )
                ...
                if len(df_scene_median) > 0:
                    ...
                    difference = df_scene_median["prediction"].astype(
                        int
                    ) - df_scene_median["validation"].astype(int)
                else:
                    y_true = [0]
                    y_pred = [100]
                    difference = 0
                ...
                c1 = df_scene_median[["validation", "prediction"]].plot(
                    kind="bar", width=0.5, ax=c
                )
                c2 = plt.plot(
                    [-10, 100], [difference, difference], "-", color="red", lw=0.5
                )
                ...
                # other
                fig.savefig(
                    folder + "/image/results_median_" + str(model_short) + ".png"
                )

                # save dataframe
                df_scene_median.to_csv(
                    folder
                    + "/csv/df_prediction_"
                    + str(model_short)
                    + "_median_scene.csv"
            ...
            for model in self.predictions:
                ...
                for i, date in enumerate(self.predict_dates):
                    ...
                    for j, roi in enumerate(rois):
                        ...
                        roi = roi.replace("date", date.strftime("%Y-%m-%d")).replace(
                            "sensor", self.sensor
                        )
                        ...
                        try:
                            ...
                            lons_lats_attributes = np.hstack(
                                (extra_attributes, lons_lats_attributes)
                            )

                            # append new data from roi to dataframe
                            df_true = df_true.append(
                                pd.DataFrame(
                                    data=lons_lats_attributes, columns=df_columns
                                ).infer_objects(),
                                ignore_index=True,
                            )
                    ...
                    if has_roi:
                        ...
                        # prepare the datas
                        df_true = df_true[df_true["label"] != 2.0][
                            ["pixel", "lat", "lon", "label"]
                        ]

                        # get rois label list
                        array_labels = (
                            df_true.groupby("label").count().values[:, 0].astype(str)
                        )

                        # prepare model data
                        df_pred = self.predictions[model][
                            ["lat", "lon", "label_predicted_" + str(i)]
                        ].rename(columns={"label_predicted_" + str(i): "label"})

                        # merge true and pred dataframes, finding matched pair of
                        ...
                        # get predictions on test set
                        y_true, y_pred = merged["label_true"].values.astype(
                            int
                        ), merged["label_pred"].values.astype(int)
                        measures = misc.concordance_measures(
                            metrics.confusion_matrix(y_true, y_pred), y_true, y_pred
                        )
    ...
    def save_timeseries_plot(self, df: pd.DataFrame, path: str, join: bool = False):
        ...
        if join:
            ...
            for i, attribute in enumerate(attributes):
                ax.plot(
                    ...
                    linestyle=linestyle[random.randint(0, len(linestyle) - 1)],
                )
            ax.legend(
                borderaxespad=0.0,
                fancybox=True,
        ...
        # else, grid plot
            ...
            for i, attribute in enumerate(attributes):
                ax = fig.add_subplot(
                    math.ceil(len(attributes) / columns), columns, i + 1
                )
                ax.grid(True, linestyle="dashed", color="#a0a0a0", linewidth=0.1)
    ...
    def save_attributes_pairplot(self, df: pd.DataFrame, path: str):
        ...
        # creating the plot
        plot = sns.pairplot(
            df[df["date"] == next(iter(g_dates))],
            vars=attributes,
            diag_kind="kde",
            plot_kws=dict(edgecolor="none", linewidth=0.2, s=1),
            corner=True,
        )
        plot.savefig(path)
    ...
    def save_results_plot(self, df: pd.DataFrame, path: str):
        ...
        rmse_ticks = np.arange(
            0, max(df["rmse"]), max(df["rmse"]) / (len(acc_ticks) / 2)
        ).astype(int)
        ...
        for i, model in enumerate(g_models):
            ...
            data = (
                df[df["model"] == model]
                .pivot(index="date_predicted", columns="type", values=["acc", "rmse"])
                .reset_index()
            )
    ...
    def save_image_tiff(
        ...
        bands = [
            ...
            self.sensor_params["green"],
            self.sensor_params["blue"],
            self.sensor_params["nir"],
            self.sensor_params["swir"],
        ] + attributes
        if self.sensor_params["sensor"] == "landsat578":
            sensor = image.get(self.sensor_params["property_id"]).getInfo()
            if "LT05" in sensor:
                bands = [
                    gee.get_sensor_params("landsat5")["red"],
                    gee.get_sensor_params("landsat5")["green"],
                    gee.get_sensor_params("landsat5")["blue"],
                    gee.get_sensor_params("landsat5")["nir"],
                    gee.get_sensor_params("landsat5")["swir"],
                ] + attributes
            elif "LE07" in sensor:
                bands = [
                    gee.get_sensor_params("landsat7")["red"],
                    gee.get_sensor_params("landsat7")["green"],
                    gee.get_sensor_params("landsat7")["blue"],
                    gee.get_sensor_params("landsat7")["nir"],
                    gee.get_sensor_params("landsat7")["swir"],
                ] + attributes
            elif "LC08" in sensor:
                bands = [
                    gee.get_sensor_params("landsat")["red"],
                    gee.get_sensor_params("landsat")["green"],
                    gee.get_sensor_params("landsat")["blue"],
                    gee.get_sensor_params("landsat")["nir"],
                    gee.get_sensor_params("landsat")["swir"],
                ] + attributes
        ...
        imageRGB = image.select(
            [
                self.sensor_params["red"],
                self.sensor_params["green"],
                self.sensor_params["blue"],
            ]
        ).visualize(min=options["min"], max=options["max"])
        imageFCO = image.select(
            [
                self.sensor_params["nir"],
                self.sensor_params["red"],
                self.sensor_params["green"],
        ...
        try:
            ...
            image_download_url = image.select(bands).getDownloadUrl(
                {
        ...
        # Second try, save in Google Drive
            ...
            for band in bands:
                task = ee.batch.Export.image.toDrive(
                    ...
                    folder=folderName,
                    description=date.strftime("%Y-%m-%d") + "_" + str(band),
                    region=self.geometry,
                )
                task.start()
                print(task.status())
        ...
        try:
            ...
            image_download_url = imageRGB.getDownloadUrl(
                {
                    "name": date.strftime("%Y-%m-%d") + "_rgb",
                    "filePerBand": False,
                    "region": self.geometry,
                }
            )
        ...
        # Second try, save in Google Drive
            ...
            task = ee.batch.Export.image.toDrive(
                image=imageRGB,
                folder=folderName,
                description=date.strftime("%Y-%m-%d") + "_rgb",
                region=self.geometry,
            )
            task.start()
            print(task.status())
        ...
        try:
            ...
            image_download_url = imageFCO.getDownloadUrl(
                {
                    "name": date.strftime("%Y-%m-%d") + "_fco",
                    "filePerBand": False,
                    "region": self.geometry,
                }
            )
        ...
        # Second try, save in Google Drive
            ...
            task = ee.batch.Export.image.toDrive(
                image=imageFCO,
                folder=folderName,
                description=date.strftime("%Y-%m-%d") + "_fco",
                region=self.geometry,
            )
            task.start()
            print(task.status())
    ...
    def save_image(
        ...
        imageIO = PIL.Image.open(
            BytesIO(
                requests.get(
                    image.select(bands).getThumbUrl(options), timeout=60
                ).content
            )
        )
        imageIO.save(path)
    ...
    def save_image_collection(
        self, path: str, bands: list = None, options: dict = {"min": 0, "max": 3000}
    ):
        ...
        for date in self.dates_timeseries_interval:
            self.save_image(
                image=self.clip_image(self.extract_image_from_collection(date=date)),
                path=path + "/" + date.strftime("%Y-%m-%d") + ".png",
                bands=bands,
                options=options,
            )
