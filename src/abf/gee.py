...
def extract_latitude_longitude_pixel(
    ...
    coordinates = image.addBands(ee.Image.pixelLonLat()).select(
        ["longitude", "latitude"] + bands
    )
    coordinates = coordinates.reduceRegion(
    ...
    for band in bands:
        band_values.append(
            np.array(ee.List(coordinates.get(band)).getInfo(), dtype=np.float64)
        )
    ...
    result[0] = np.array(
        ee.List(coordinates.get("longitude")).getInfo(), dtype=np.float64
    )
    result[1] = np.array(
        ee.List(coordinates.get("latitude")).getInfo(), dtype=np.float64
    )
...
def get_sensor_collections(
    geometry: ee.Geometry, sensor: str = "landsat", dates: list = None
):
    ...
    if sensor == "sentinel":
        collection = (
            ...
            .filter(ee.Filter.lte("NODATA_PIXEL_PERCENTAGE", "less_than", 100))
            .sort("system:time_start", True)
            .map(apply_masks_sentinel)
        )
        collection_water = (
            ee.ImageCollection("COPERNICUS/S2_SR")
            .filterBounds(geometry)
            .filterMetadata("CLOUDY_PIXEL_PERCENTAGE", "less_than", 10)
            .filter(ee.Filter.lte("NODATA_PIXEL_PERCENTAGE", 0))
            .sort("system:time_start", True)
            .map(apply_masks_sentinel)
        )
    ...
    # Landsat-5/7/8 - Merge
        ...
        collection5 = (
            ee.ImageCollection("LANDSAT/LT05/C01/T1_SR")
            .filterDate(
                get_sensor_params("landsat5")["start"],
                get_sensor_params("landsat5")["end"],
            )
            .filterBounds(geometry)
            .filterMetadata("CLOUD_COVER", "less_than", 100)
            .sort("system:time_start", True)
        ...
        collection7 = (
            ...
            .filterDate(
                get_sensor_params("landsat5")["end"],
                get_sensor_params("landsat")["start"],
            )
            .filterBounds(geometry)
            .filterMetadata("CLOUD_COVER", "less_than", 100)
            .sort("system:time_start", True)
        ...
        collection8 = (
            ...
            .sort("system:time_start", True)
            .select(["B1", "B2", "B3", "B4", "B5", "B6", "pixel_qa"])
            .map(apply_masks_landsat)
        )

        # Landsat Merge
        collection = collection8.merge(collection7.merge(collection5)).sort(
    ...
    if dates:
        ...
        if not collection_water:
            collection_water = collection.filterDate(
                dt.strftime(
                    dt.strptime(dates[0], "%Y-%m-%d") - td(days=180), "%Y-%m-%d"
                ),
                dt.strftime(
                    dt.strptime(dates[1], "%Y-%m-%d") + td(days=180), "%Y-%m-%d"
                ),
            ).sort("system:time_start", True)
...
def mask_cloud_shadow(image, sensor: str):
    ...
    # Absolute Values: https://prd-wret.s3.us-west-2.amazonaws.com/assets/palladium/production/atoms/files/LSDS-1370_L4-7_SurfaceReflectance-LEDAPS_ProductGuide-v2.pdf
        ...
        return (qa.bitwiseAnd(1 << 5).eq(0) and (qa.bitwiseAnd(1 << 7).eq(0))) or (
...
def mask_water(image, sensor: str):
    ...
    # Values: https://developers.google.com/earth-engine/datasets/catalog/MODIS_006_MOD09GA
        ...
        qa = image.select("state_1km")
        return (qa.bitwiseAnd(1 << 3).neq(0)) and (
            qa.bitwiseAnd(1 << 4).eq(0) or qa.bitwiseAnd(1 << 5).eq(0)
        )
...
def apply_masks(image, params: dict):
    ...
    cloud = (
        ee.Image(1)
        .updateMask(mask_cloud_shadow(image, params["sensor"]).Not())
        .rename("cloud")
    )
    nocloud = blank.updateMask(mask_cloud_shadow(image, params["sensor"])).rename(
        "nocloud"
    )
    ...
    mndwi = (
        image.expression(
            ...
            {
                "swir": image.select(params["swir"]).multiply(0.0001),
                "green": image.select(params["green"]).multiply(0.0001),
            },
        )
        .rename("mndwi")
        .cast({"mndwi": "double"})
    ...
    ndvi = (
        image.expression(
            "(nir - red) / (nir + red)",
            {
                "nir": image.select(params["nir"]).multiply(0.0001),
                "red": image.select(params["red"]).multiply(0.0001),
            },
        )
        .rename("ndvi")
        .cast({"ndvi": "double"})
    ...
    sabi = (
        image.expression(
            "(nir - red) / (blue + green)",
            {
                "nir": image.select(params["nir"]).multiply(0.0001),
                "red": image.select(params["red"]).multiply(0.0001),
                "blue": image.select(params["blue"]).multiply(0.0001),
                "green": image.select(params["green"]).multiply(0.0001),
            },
        )
        .rename("sabi")
    ...
    fai = (
        image.expression(
            "nir - (red + (swir - red) * ((c_nir - c_red) / (c_swir - c_red)))",
            {
                "swir": image.select(params["swir"]).multiply(0.0001),
                "nir": image.select(params["nir"]).multiply(0.0001),
                "red": image.select(params["red"]).multiply(0.0001),
    ...
    return image.addBands(
        [water, water_nocloud, cloud, nocloud, mndwi, ndvi, sabi, fai]
    )  # , label
...
def get_image_min_max(
    image: ee.Image, geometry: ee.Geometry, scale: int = None, tile_scale: int = 16
):
    if scale is None:
